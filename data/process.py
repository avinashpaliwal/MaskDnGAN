...
TENSORFLOW_RESIZE_BILINEAR = 0
if TENSORFLOW_RESIZE_BILINEAR == 1:
...
def demosaic(bayer_images):
  ...
  shape = [shape[1] * 2 - 1, shape[2] * 2 - 1]

  red = bayer_images[Ellipsis, 0:1]
  upsamplebyX = nn.Upsample(size=shape, mode='bilinear', align_corners=True)
  red = nn.functional.pad(upsamplebyX(red.permute(0, 3, 1, 2)), (0, 1, 0, 1), mode='replicate').permute(0, 2, 3, 1)

  green_red = bayer_images[Ellipsis, 1:2]
  green_red = torch.flip(green_red, dims=[2]) # Flip left-right
  green_red = nn.functional.pad(upsamplebyX(green_red.permute(0, 3, 1, 2)), (0, 1, 0, 1), mode='replicate').permute(0, 2, 3, 1)
  green_red = torch.flip(green_red, dims=[2]) # Flip left-right
  green_red = SpaceToDepth_fact2(green_red.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)

  green_blue = bayer_images[Ellipsis, 2:3]
  green_blue = torch.flip(green_blue, dims=[1]) # Flip up-down
  green_blue = nn.functional.pad(upsamplebyX(green_blue.permute(0, 3, 1, 2)), (0, 1, 0, 1), mode='replicate').permute(0, 2, 3, 1)
  green_blue = torch.flip(green_blue, dims=[1]) # Flip up-down
  ...
  blue = torch.flip(torch.flip(blue, dims=[2]), dims=[1])
  blue = nn.functional.pad(upsamplebyX(blue.permute(0, 3, 1, 2)), (0, 1, 0, 1), mode='replicate').permute(0, 2, 3, 1)
  blue = torch.flip(torch.flip(blue, dims=[2]), dims=[1])
